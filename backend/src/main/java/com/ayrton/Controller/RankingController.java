...
/*
    ...
    public ResponseEntity<?> findAll(@RequestParam int page, @RequestParam int size) {
        try {
            ...
            Page<RankingDto> result = rankingBusiness.findAll(pageable);
            return ResponseEntity.ok(result);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al obtener los rankings: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> getById(@PathVariable Long id) {
        try {
            RankingDto result = rankingBusiness.getById(id);
            return ResponseEntity.ok(result);
        } catch (NoSuchElementException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("Ranking no encontrado con ID: " + id);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al obtener el ranking: " + e.getMessage());
        }
    }

    // 3. Crear un nuevo ranking
    @PostMapping("/create")
    public ResponseEntity<?> create(@RequestBody RankingDto rankingDto) {
        try {
            RankingDto result = rankingBusiness.save(rankingDto);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al crear el ranking: " + e.getMessage());
        }
    }

    // 4. Eliminar un ranking por su ID
    @DeleteMapping("/{id}")
    public ResponseEntity<?> delete(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error al eliminar el ranking: " + e.getMessage());
        }
    }
 */
}